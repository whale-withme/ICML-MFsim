...
class StateTransitionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_dist_batch = torch.tensor([
            row['batch_ratio_pos'], row['batch_ratio_neu'], row['batch_ratio_neg']
        ], dtype=torch.float32)

        target_dist_sum = torch.tensor([
            row['cum_ratio_pos'], row['cum_ratio_neu'], row['cum_ratio_neg']
        ], dtype=torch.float32)
        ...
        return {
            ...
            "target_dist_batch": target_dist_batch,
            "target_dist_sum": target_dist_sum,
            "step_idx": torch.tensor(idx, dtype=torch.long)
        }
